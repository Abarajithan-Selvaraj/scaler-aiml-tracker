...
import { Calendar, Plane, Shield, Clock, Moon, CheckCircle, Video, FileCode, HelpCircle, Check } from 'lucide-react';
                    ...
                    {[group.amBlock, group.pmBlock].map((block) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* Focus Items List with Sub-Components */}
                            ...
                            {blockSyllabusItems.length > 0 ? (
                              blockSyllabusItems.map((item) => (
                                <div key={item.id} className="p-2 rounded-lg bg-slate-950/60 border border-slate-800/60 space-y-1.5">
                                  ...
                                  {/* Sub-Components Pills in Timetable Block Card */}
                                  {item.type === 'Class' && (
                                    <div className="flex flex-wrap gap-1 pt-1">
                                      <button
                                        type="button"
                                        onClick={() => toggleSubComponentCompletion(item.id, 'video')}
                                        className={`flex items-center space-x-1 px-1.5 py-0.5 rounded text-[9px] font-semibold border transition-colors ${
                                          item.videoCompleted
                                      ...
                                      >
                                        ...
                                        <span>Video</span>
                                        {item.videoCompleted && <Check className="w-2.5 h-2.5 ml-0.5 text-indigo-300" />}
                                      </button>
                                      ...
                                      >
                                        ...
                                        {item.assignmentCompleted && <Check className="w-2.5 h-2.5 ml-0.5 text-purple-300" />}
                                      </button>
                                      ...
                                      >
                                        ...
                                        <span>Add. Prob.</span>
                                        {item.additionalProblemsCompleted && <Check className="w-2.5 h-2.5 ml-0.5 text-amber-300" />}
                                      </button>
                                    </div>
                                  )}
                                </div>
